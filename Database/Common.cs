...
namespace DBClass
{
    public class Common
    {
        ...
        public static string getNoByTitle(string title)
        {
            ...
            {
                match = Regex.Match(title, @"MA600-/d{2}[A-Za-z0-9_ -]+");
                if (match.Success)
                {
                    re = match.Groups[0].Value;
                }
                ...
                {
                    match = Regex.Match(title, @"(SB|JWC|CFED|EMC|CAD|TB|CEB|CSL|BHT|GHN|SL|SA|SIL)[A-Za-z0-9_ -]+");
                    if (match.Success)
                    {
                        re = match.Groups[0].Value;
                    }
        ...
        public static string getFromName(string title)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        match = Regex.Match(title, @"\d{4}JSK\d{3}");
                        if (match.Success)
                        {
                            re = "机务处";
                        }
